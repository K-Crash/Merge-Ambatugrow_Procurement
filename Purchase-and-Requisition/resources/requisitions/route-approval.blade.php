    ...
    <form method="POST" action="{{ route('requisitions.route.store', $requisition) }}" class="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-5">
        ...
        <div class="card p-5">
            ...
            <div class="flex items-center justify-end gap-2 mt-6 pt-5 border-t border-slate-100">
                ...
                <button type="submit" class="btn-primary px-5 py-2 text-sm rounded-md text-white font-semibold">Submit for approval</button>
            </div>
        </div>
...
<script>
    function routeForm() {
        return {
            ...
            users: @json(collect($approvers)->flatten(1)->values()->map(fn($u) => ['id' => $u->id, 'name' => $u->name, 'job_title' => $u->job_title, 'role' => $u->role])),
            steps: @json($steps->map(fn($s) => [
